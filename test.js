...
const addon = require('./lib/WeWorkFinanceSdk_Node.node');

const corpid = 'ww6d13a63fdc80988d';
const secret = 'Lw6yMb_W_s-dZpubVWpa23FTcJBtlSGIHpXGNEaI33s';
...
function decryptData(encrypt_random_key, publickey_ver) {
  ...
  const privateKey = fs.readFileSync(
    path.resolve(__dirname, './lib/private.key')
  );
...
async function saveMsgs(msgs) {
  for (let i in msgs) {
    ...
    if (allowedMsgTypes.includes(msg.msgtype)) {
      ...
      const ret = addon.getMediaData(
        sdkfileid,
        '',
        '',
        60,
        path.resolve(__dirname, `../libs/temp/${md5sum}`)
      );
